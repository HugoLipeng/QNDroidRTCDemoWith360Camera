...
public class PGSkinUtils {
    ...
    public PGSkinUtils(Context context) {
        mContext = context;
        mPGSkinUtils = new PGPrettifySDK(context, false);
    }
     ...
     * 初始化
     */
    private void initEngine(boolean isInitEGL,boolean isFront) {
        /**
         * FAQ
         * 初始化引擎时卡住或闪退
         *
...
//        mPGSkinUtils.InitialiseEngine(APP_KEY, isInitEGL); // 不带贴纸SDK初始化引擎
//        mPGSkinUtils.SetSizeForAdjustInput(mCameraInfo.previewHeight, mCameraInfo.previewWidth);// 调整输入帧的宽高。Android默认摄像头采集数据是横向的，因此宽高对换
        ...
        if (isFront){
            //mPGSkinUtils.SetOutputOrientation(PGSkinPrettifyEngine.PG_Orientation.PG_OrientationRightRotate270);
        }else {
            mPGSkinUtils.SetOutputOrientation(PGSkinPrettifyEngine.PG_Orientation.PG_OrientationRightRotate90);
        }
        mPGSkinUtils.SetSkinColor(mPinkValue, mWhitenValue, mReddenValue);//设置美颜参数
        mPGSkinUtils.Switch2DStickerWH(true);
        /**
         * FAQ
         * Android美颜有没有推荐的参数
         *
     ...
     */
    public void frameProcess(byte[] data, int textureId, boolean isFirstFrame, boolean isInitEGL,boolean isFront) {
        /**
         * FAQ
         * 屏幕花屏
         *
        ...
        if (isFirstFrame) initEngine(isInitEGL,isFront);//  在第一帧视频到来时，初始化，指定需要的输出大小以及方向
        /**
         * FAQ
